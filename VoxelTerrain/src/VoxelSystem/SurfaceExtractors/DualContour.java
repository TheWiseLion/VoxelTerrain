...
public class DualContour implements SurfaceExtractor{
	...
	public List<SurfacePoint> extractSurface(Vector3f point, VoxelGrid hg, float scale) { 
		//TODO: iterate over smallest axis
		BoundingBox bb = new BoundingBox();
		bb.setXExtent(scale);
		bb.setYExtent(scale);
		bb.setZExtent(scale);
		float threshold = (float) (Math.sqrt(scale*scale + scale*scale + scale*scale) / 1000.0);
...
//					if(x+1 == y+1,z+1)
	...
	private void setCubeEdges(int x, int y, int z, Vector3f [] cP,float scale){
		cP[0].y = cP[1].y = cP[4].y = cP[5].y = (float)y*scale;
		cP[2].y = cP[3].y = cP[6].y = cP[7].y = (float)(y+1)*scale;
		
		cP[0].x = cP[3].x = cP[4].x = cP[7].x = (float)x*scale;
		cP[1].x = cP[2].x = cP[5].x = cP[6].x = (float)(x+1)*scale;
		
		cP[0].z = cP[1].z = cP[2].z = cP[3].z = (float)z*scale;
		cP[4].z = cP[5].z = cP[6].z = cP[7].z = (float)(z+1)*scale;
	}
